...
%Solving 1st order derivative numerically using 6th order compact finite difference method
clc
clear all

format long
m=[ 100 200 300 500 1000 2000];%number of grid points
i=1;
j=1;
Err1=zeros(6,1);
delx=zeros(6,1);
...
df=compact_1st_6th_order( f,m(j),delx(j));
plot(x1,df,'k-*');
xlabel('x');
ylabel('df/dx');
